const CACHE_NAME='nextmatch-v20';
const APP_SHELL=[
  '/GPTomgeving/',
  ...
  '/GPTomgeving/assets/club-mobile-v18.js',
  '/GPTomgeving/assets/club-intelligence-v18.css',
  '/GPTomgeving/assets/club-intelligence-v18.js',
  '/GPTomgeving/assets/club-os-v20.css',
  '/GPTomgeving/assets/club-os-v20.js',
...
self.addEventListener('fetch',event=>{
  const request=event.request;if(request.method!=='GET')return;const url=new URL(request.url);if(url.origin!==self.location.origin)return;if(url.pathname.includes('/supabase/')||url.pathname.includes('/auth/'))return;
  if(request.mode==='navigate'){event.respondWith(fetch(request,{cache:'no-store'}).then(response=>{const copy=response.clone();caches.open(CACHE_NAME).then(cache=>cache.put('/GPTomgeving/index.html',copy));return response}).catch(()=>caches.match('/GPTomgeving/index.html')));return}
